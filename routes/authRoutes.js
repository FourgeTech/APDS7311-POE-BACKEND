...
//User login route
router.post('/customer/login', loginCustomerValidation, (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    authController.loginCustomer(req, res);
});

//User login route
router.post('/employee/login', loginEmployeeValidation, (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    authController.loginEmployee(req, res);
});
